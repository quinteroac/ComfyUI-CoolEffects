...
WIDGET_PATH = PACKAGE_ROOT / "web" / "frosted_glass_effect.js"
...
def test_mount_frosted_glass_widget_creates_canvas_and_uses_frosted_glass_shader_source():
    script = f"""
import {{ mount_frosted_glass_effect_widget_for_node }} from "{WIDGET_URL}";
...
const widget_state = await mount_frosted_glass_effect_widget_for_node({{
    node,
    document_ref,
...
"""
    ...
    assert output["shaderCalls"] == ["frosted_glass"]
...
def test_registers_frosted_glass_extension_and_mounts_widget_on_node_create():
    script = f"""
...
await captured_extension.beforeRegisterNodeDef(NodeType, {{ name: "CoolFrostedGlassEffect" }});
...
console.log(JSON.stringify({{
    ...
    previewEffect: node.__cool_frosted_glass_widget_state.preview_state.preview_descriptor.effect_name,
}}));
"""
    output = _run_node_module(script)
    assert output["extensionName"] == output["expectedName"]
    assert output["widgetCount"] == 1
    assert output["previewEffect"] == "frosted_glass"


def test_frosted_glass_widget_changes_update_preview_uniforms_without_shader_reload():
    script = f"""
...
const controller = node.__cool_frosted_glass_widget_state.preview_state.preview_controller;
const uniform_calls = [];
...
let did_throw = false;
try {{
    node.onWidgetChanged("frost_intensity", 0.82);
    node.onWidgetChanged("blur_radius", 0.03);
    node.onWidgetChanged("uniformity", 0.35);
    node.onWidgetChanged("tint_temperature", -0.42);
    node.onWidgetChanged("condensation_rate", 0.65);
}} catch (_error) {{
    did_throw = true;
}}
...
"""
    ...
    assert output["uniformCalls"] == [
        ["u_frost_intensity", 0.82],
        ["u_blur_radius", 0.03],
        ["u_uniformity", 0.35],
        ["u_tint_temperature", -0.42],
        ["u_condensation_rate", 0.65],
    ]
    assert output["uniforms"]["u_frost_intensity"] == 0.82
    assert output["uniforms"]["u_blur_radius"] == 0.03
    assert output["uniforms"]["u_uniformity"] == 0.35
    assert output["uniforms"]["u_tint_temperature"] == -0.42
    assert output["uniforms"]["u_condensation_rate"] == 0.65
