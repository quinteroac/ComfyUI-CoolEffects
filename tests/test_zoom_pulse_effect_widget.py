...
def test_zoom_pulse_widget_changes_update_preview_uniforms_without_shader_reload():
    script = f"""
...
const controller = node.__cool_zoom_pulse_widget_state.preview_state.preview_controller;
const uniform_calls = [];
...
let did_throw = false;
try {{
    node.onWidgetChanged("pulse_amp", 0.12);
    node.onWidgetChanged("pulse_speed", 5.5);
}} catch (_error) {{
    did_throw = true;
}}
...
"""
    ...
    assert output["uniforms"]["u_pulse_amp"] == 0.12
    assert output["uniforms"]["u_pulse_speed"] == 5.5
