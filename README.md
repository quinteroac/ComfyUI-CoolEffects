...
- `CoolVideoMixer`: loads supported video files from a directory in case-insensitive filename order, with `transition_type` (`crossfade`/`hard_cut`/`fade_to_black`) and `transition_duration` controls.
